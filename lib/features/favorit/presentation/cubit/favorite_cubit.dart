    ...
    try {
      ...
      emit(
        const FavoriteFailure(
          errorMessage: 'Unable to load favorites. Please try again.',
        ),
      );
    }
  }
  ...
  Future<void> addToFavorite(int productId) async {
    ...
    try {
      ...
      emit(
        AddFavoriteFailure(
          productId: productId,
          errorMessage: 'Unable to add to favorites. Please try again.',
        ),
      );
    } finally {
  ...
  Future<void> deleteFavorite(int productId) async {
    ...
    try {
      ...
      emit(
        DeleteFavoriteFailure(
          productId: productId,
          errorMessage: 'Unable to delete from favorites. Please try again.',
        ),
      );
    } finally {
      _isUpdatingFavorite = false;
